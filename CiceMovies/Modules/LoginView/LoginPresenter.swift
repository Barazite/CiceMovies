...
class LoginPresenter: LoginPresenterProtocol, ObservableObject{
    ...
    private var arePasswordEqualValidPublisher: AnyPublisher<Bool, Never>{
        Publishers.CombineLatest($password, $passwordAgain)//Pueden ser hasta un maximo de 4
            .debounce(for: 0.8, scheduler: RunLoop.main)
    ...
    init(){
        ...
        isNameValidPublisher
            ...
            .map{ valid in
                valid ? "" : "El nombre debe contener un mínimo de 5 caracteres"
        ...
        isLastnameValidPublisher
            .receive(on: RunLoop.main)
            .map{ valid in
                valid ? "" : "El apellido debe contener un mínimo de 5 caracteres"
